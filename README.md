...
| **the α = 0 rerun of Section 6** (test vs. plain pump at pure projection: 17/158/1, +17.9 points) | `cd campaign_scip && python agg_alpha0.py` | reads `results_alpha0.txt` (job `job30_alpha0.sh`, 1,760 runs); report `REPORT_alpha0_it.md` |
| **the timed completion of Section 6** (share of wall-clock time in the completion step, mixed vs. pure) | `cd campaign_scip && python agg_lpft.py` | reads `results_lpft.txt` (job `job31_lpft.sh`, 2,640 runs with `scip_f6` = the campaign binary plus `scip/scip_patch_lpfixtime.py`, built by `scip/scip_build_f6.sh`); report `REPORT_lpft_it.md` |
| **the adaptive moat of Section 7** (moat from the observed rounding gap; three rules) | `cd campaign_gurobi && python agg_adapt.py` | reads `results/eval50_adapt*.res`, `eval90_adapt*.res` (job `job27_adapt.sh`, modes `test-adapt`/`completion-adapt` of `fp_target.py`, 12,060 runs); report `REPORT_adapt_it.md` |
